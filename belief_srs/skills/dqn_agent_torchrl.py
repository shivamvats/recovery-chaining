...
from torchrl.objectives import DQNLoss, DistributionalDQNLoss, SoftUpdate, HardUpdate
...
class DQNAgentTorchrl:
    def __init__(self, make_env_fn, cfg, wandb_logger):
        ...
        # TODO
        # double DQN https://github.com/pytorch/rl/issues/1722
    ...
    def _init(self, env):
        ...
        if rl_cfg.recurrent:
            ...
            # self.init_weights(module, gain=np.sqrt(2))
            ...
            qval = QValueActor(
                module=nn.Sequential(feature, mlp),
                action_space=env.action_spec,
                in_keys=["pixels"],
            )
        ...
        greedy_module = EGreedyModule(
            annealing_num_steps=int(
                rl_cfg.exploration_fraction * rl_cfg.total_timesteps
            ),
            eps_init=rl_cfg.exploration_init_eps,
            eps_end=rl_cfg.exploration_final_eps,
